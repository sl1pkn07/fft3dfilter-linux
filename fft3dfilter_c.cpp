...
//-----------------------------------------------------------------------------------------
//
void ApplyWiener2D_C(fftw_complex *outcur, int outwidth, int outpitch, int bh, 
...
//-----------------------------------------------------------------------------------------
//
void ApplyWiener3D2_C(fftw_complex *outcur, fftw_complex *outprev, 
...
//-----------------------------------------------------------------------------------------
//
void ApplyPattern3D2_C(fftw_complex *outcur, fftw_complex *outprev, 
...
//-----------------------------------------------------------------------------------------
//
void ApplyWiener3D3_C(fftw_complex *outcur, fftw_complex *outprev, fftw_complex *outnext, 
					int outwidth, int outpitch, int bh, int howmanyblocks, 
					float sigmaSquaredNoiseNormed, float beta)
{
	// this function take 25% CPU time and may be easy optimized for AMD Athlon 3DNOW assembler
...
//-----------------------------------------------------------------------------------------
//
void ApplyPattern3D3_C(fftw_complex *outcur, fftw_complex *outprev, fftw_complex *outnext, 
					 int outwidth, int outpitch, int bh, int howmanyblocks, 
...
//-----------------------------------------------------------------------------------------
//
void ApplyWiener3D4_C(fftw_complex *outcur, fftw_complex *outprev2, fftw_complex *outprev, 
					fftw_complex *outnext, int outwidth, int outpitch, int bh, 
					int howmanyblocks, float sigmaSquaredNoiseNormed, float beta)
{
	// dft with 4 points
	// this function take 25% CPU time and may be easy optimized for AMD Athlon 3DNOW assembler
...
//-----------------------------------------------------------------------------------------
//
void ApplyPattern3D4_C(fftw_complex *outcur, fftw_complex *outprev2, fftw_complex *outprev, 
					 fftw_complex *outnext, int outwidth, int outpitch, int bh, 
					 int howmanyblocks, float* pattern3d, float beta)
{
	// dft with 4 points
...
//-----------------------------------------------------------------------------------------
//
void ApplyKalmanPattern_C( fftw_complex *outcur, fftw_complex *outLast, 
						fftw_complex *covar, fftw_complex *covarProcess,
						int outwidth, int outpitch, int bh, int howmanyblocks,  
...
//-----------------------------------------------------------------------------------------
//
void ApplyKalman_C( fftw_complex *outcur, fftw_complex *outLast, fftw_complex *covar, 
				 fftw_complex *covarProcess, int outwidth, int outpitch, int bh, 
...
//-------------------------------------------------------------------------------------------
//
void Sharpen_C(fftw_complex *outcur, int outwidth, int outpitch, int bh, 
...
//-------------------------------------------------------------------------------------------
//
void Sharpen_degrid_C(fftw_complex *outcur, int outwidth, int outpitch, int bh, 
			 int howmanyblocks, float sharpen, float sigmaSquaredSharpenMin, 
			 float sigmaSquaredSharpenMax, float *wsharpen, 
			 float degrid, fftw_complex *gridsample, float dehalo, float *wdehalo, float ht2n)
{
	int h,w, block;
	float psd;
	float sfact;
...
//-----------------------------------------------------------------------------------------
//-----------------------------------------------------------------------------------------
void ApplyWiener2D_degrid_C(fftw_complex *outcur, int outwidth, int outpitch, int bh, 
				   int howmanyblocks, float sigmaSquaredNoiseNormed, float beta, 
				   float sharpen, float sigmaSquaredSharpenMin, 
				   float sigmaSquaredSharpenMax, float *wsharpen, 
				   float degrid, fftw_complex *gridsample, float dehalo, float *wdehalo, float ht2n)
...
//-----------------------------------------------------------------------------------------
//
void ApplyWiener3D2_degrid_C(fftw_complex *outcur, fftw_complex *outprev, 
					int outwidth, int outpitch, int bh, int howmanyblocks, 
					float sigmaSquaredNoiseNormed, 	float beta, 
					float degrid, fftw_complex *gridsample)
{
	// this function take 25% CPU time and may be easy optimized for AMD Athlon 3DNOW assembler
	// return result in outprev
	float lowlimit = (beta-1)/beta; //     (beta-1)/beta>=0
...
//-----------------------------------------------------------------------------------------
//
void ApplyWiener3D3_degrid_C(fftw_complex *outcur, fftw_complex *outprev, fftw_complex *outnext, 
					int outwidth, int outpitch, int bh, int howmanyblocks, 
					float sigmaSquaredNoiseNormed, float beta, 
					float degrid, fftw_complex *gridsample)
{
...
//-----------------------------------------------------------------------------------------
//
void ApplyWiener3D4_degrid_C(fftw_complex *outcur, fftw_complex *outprev2, fftw_complex *outprev, 
					fftw_complex *outnext, int outwidth, int outpitch, int bh, 
					int howmanyblocks, float sigmaSquaredNoiseNormed, float beta, 
					float degrid, fftw_complex *gridsample)
{
...
//-------------------------------------------------------------------------------------------
//
void ApplyPattern2D_degrid_C(fftw_complex *outcur, int outwidth, int outpitch, int bh, 
...
//-----------------------------------------------------------------------------------------
//
void ApplyPattern3D2_degrid_C(fftw_complex *outcur, fftw_complex *outprev, 
					 int outwidth, int outpitch, int bh, 
					 int howmanyblocks, float *pattern3d, float beta,
					 float degrid, fftw_complex *gridsample)
{
...
//-----------------------------------------------------------------------------------------
//
void ApplyPattern3D3_degrid_C(fftw_complex *outcur, fftw_complex *outprev, fftw_complex *outnext, 
					 int outwidth, int outpitch, int bh, int howmanyblocks, 
					 float *pattern3d, float beta,
					 float degrid, fftw_complex *gridsample)
{
...
//-----------------------------------------------------------------------------------------
//
void ApplyPattern3D4_degrid_C(fftw_complex *outcur, fftw_complex *outprev2, fftw_complex *outprev, 
					 fftw_complex *outnext, int outwidth, int outpitch, int bh, 
					 int howmanyblocks, float* pattern3d, float beta,
					 float degrid, fftw_complex *gridsample)
{
...
//-----------------------------------------------------------------------------------------
//
void ApplyPattern3D5_degrid_C(fftw_complex *outcur, fftw_complex *outprev2, fftw_complex *outprev, 
					fftw_complex *outnext, fftw_complex *outnext2, int outwidth, int outpitch, int bh, 
					int howmanyblocks, float *pattern3d, float beta, 
					float degrid, fftw_complex *gridsample)
{
	// dft with 5 points
...
//-----------------------------------------------------------------------------------------
//
void ApplyWiener3D5_degrid_C(fftw_complex *outcur, fftw_complex *outprev2, fftw_complex *outprev, 
					fftw_complex *outnext, fftw_complex *outnext2, int outwidth, int outpitch, int bh, 
					int howmanyblocks, float sigmaSquaredNoiseNormed, float beta, 
					float degrid, fftw_complex *gridsample)
{
...
//-----------------------------------------------------------------------------------------
//
void ApplyPattern3D5_C(fftw_complex *outcur, fftw_complex *outprev2, fftw_complex *outprev, 
					fftw_complex *outnext, fftw_complex *outnext2, int outwidth, int outpitch, int bh, 
					int howmanyblocks, float *pattern3d, float beta)
{
	// dft with 5 points
...
//-----------------------------------------------------------------------------------------
//
void ApplyWiener3D5_C(fftw_complex *outcur, fftw_complex *outprev2, fftw_complex *outprev, 
					fftw_complex *outnext, fftw_complex *outnext2, int outwidth, int outpitch, int bh, 
					int howmanyblocks, float sigmaSquaredNoiseNormed, float beta)
{
	// dft with 5 points
